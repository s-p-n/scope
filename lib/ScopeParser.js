...
var combineSourceMap = _interopRequireWildcard(_combineSourceMap);

var _convertSourceMap = require("convert-source-map");

var convertSourceMap = _interopRequireWildcard(_convertSourceMap);
...
class ScopeParser {
	...
	import(srcFilename, filename) {
		let self = this;
		let mSrcFilename = path.resolve(path.dirname(srcFilename), JSON.parse(filename));
		let mCode = fs.readFileSync(mSrcFilename, 'utf8');
		let mResult = new ScopeParser().translate(mCode, mSrcFilename, mSrcFilename.replace(/\.sc$/, '') + ".js.map", true);
		self.imported.push({ srcFilename: filename, code: mCode });
		return mResult;
	}
	...
	translate(code, srcFilename = "NO SOURCE FILE", mapFilename = "NO MAP FILE", asInclude = false) {
		...
		if (asInclude) {
			...
			traversal = self.sn([scopeRuntime, `scope.sourceMapFilename=${JSON.stringify(mapFilename)};`,
